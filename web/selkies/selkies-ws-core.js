...
export default function websockets() {
  console.log('[WS] websockets module STARTED');
  console.log('[WS] About to check isSharedMode...');
let decoder;
...
console.log('[WS] Parsing URL params...');
const urlParams = new URLSearchParams(window.location.search);
const authToken = urlParams.get('token');
console.log('[WS] authToken:', authToken);
...
console.log('[WS] isSharedMode:', isSharedMode);
...
function sanitizeAndStoreSettings(serverSettings) {
  console.log("Sanitizing and storing settings based on server payload:", serverSettings);
  const changes = {};
  ...
  console.log("sanitizeAndStoreSettings returning changes:", changes);
  return changes;
}
console.log('[WS] Loading settings from localStorage...');
...
console.log('[WS] Settings loaded, checking shared mode for resolution...');
if (isSharedMode) {
    manual_width = 1280;
    manual_height = 720;
...
console.log('[WS] Resolution settings done');
...
console.log('[WS] Registering message event listener...');
window.addEventListener('message', receiveMessage, false);
console.log('[WS] Message event listener registered');
...
function handleSettingsMessage(settings) {
  console.log('handleSettingsMessage called with:', settings);
  console.log('Current currentEncoderMode before update:', currentEncoderMode);
  ...
  if (settings.encoder !== undefined) {
    ...
    console.log('Updating encoder from', currentEncoderMode, 'to', newEncoderSetting);
    if (currentEncoderMode !== newEncoderSetting) {
        currentEncoderMode = newEncoderSetting;
        setStringParam('encoder', currentEncoderMode);
        settingsChanged = true;
        console.log('currentEncoderMode updated to:', currentEncoderMode);
...
function onDOMReady(callback) {
  if (document.readyState === 'loading') {
    ...
    console.log('[WS] Waiting for DOMContentLoaded event...');
  } else {
    console.log('[WS] DOM already loaded, running callback immediately');
    callback();
  }
}

onDOMReady(() => {
  console.log('[WS] DOMContentLoaded event fired or DOM already ready!');
  ...
  console.log('[WS] About to run preflight checks...');
  if (!runPreflightChecks()) {
    console.log('[WS] Preflight checks failed, returning early');
    return;
  }
  console.log('[WS] Preflight checks passed, continuing...');
  ...
  async function decodeAndQueueJpegStripe(startY, jpegData) {
    try {
      // Try ImageDecoder first (faster, but requires secure context)
      ...
      // Fallback: use createImageBitmap (works in non-secure context)
...
function handleDecodedFrame(frame) {
  ...
  async function initializeAudio() {
    ...
    // Check if AudioWorklet is available (requires secure context in most browsers)
    if (!audioContext.audioWorklet) {
      console.warn('AudioWorklet not available (requires HTTPS or localhost). Audio disabled.');
      return;
    }
    try {
  ...
  const urlParams = new URLSearchParams(window.location.search);
  const wsUrlOverride = window.__SELKIES_WS_URL__ || urlParams.get('ws_url');
  console.log('[WS] ===== URL CONSTRUCTION START =====');
  console.log('[WS] wsUrlOverride:', wsUrlOverride);
  console.log('[WS] window.__SELKIES_WS_URL__:', window.__SELKIES_WS_URL__);
  console.log('[WS] urlParams.get("ws_url"):', urlParams.get('ws_url'));
  console.log('[WS] Injected port:', window.__SELKIES_WS_PORT__);
  console.log('[WS] Location hostname:', window.location.hostname);
  console.log('[WS] Location href:', window.location.href);
  console.log('[WS] Location search:', window.location.search);
  let websocketEndpointURL;
  if (wsUrlOverride) {
    try {
      websocketEndpointURL = new URL(wsUrlOverride, window.location.href);
      console.log('[WS] Using ws_url param, URL:', websocketEndpointURL.href);
    } catch (err) {
      console.warn('[WS] Invalid ws_url, using fallback:', err.message);
      websocketEndpointURL = null;
    }
  }
  if (!websocketEndpointURL) {
    const wsPortOverride = window.__SELKIES_WS_PORT__ || urlParams.get('ws_port');
    const hostname = window.location.hostname;
    const port = wsPortOverride || '8080';
    const host = port ? hostname + ':' + port : hostname;
    websocketEndpointURL = new URL(ws_protocol + host + pathname);
    console.log('[WS] Using fallback, constructed URL:', websocketEndpointURL.href);
  }
  if (isTokenAuthMode) {
      websocketEndpointURL.search = `?token=${authToken}`;
  }
  if (!websocketEndpointURL.pathname.endsWith('websockets')) {
    if (!websocketEndpointURL.pathname.endsWith('/')) {
      websocketEndpointURL.pathname += '/';
    }
    websocketEndpointURL.pathname += 'websockets';
  }

  console.log('Final WebSocket URL:', websocketEndpointURL.href);
  ...
  websocket.onmessage = (event) => {
    if (event.data instanceof ArrayBuffer) {
      ...
      if (dataTypeByte === 0) {
        ...
        if (!canProcessJpeg) {
          console.log('Cannot process JPEG stripe - isSharedMode:', isSharedMode,
                      'isVideoPipelineActive:', isVideoPipelineActive,
                      'currentEncoderMode:', currentEncoderMode);
        }

        if (canProcessJpeg) {
          if (jpegDataBuffer.byteLength === 0) return;
          console.log('Processing JPEG stripe at y:', stripe_y_start, 'size:', jpegDataBuffer.byteLength);
      ...
      if (event.data === 'MODE websockets') {
        ...
        if (event.data.startsWith('{')) {
          ...
          if (obj.type === 'system_stats') window.system_stats = obj;
              ...
              console.log('Received server_settings:', obj.settings);
              ...
              // Always sync encoder mode from server settings, even if no changes
              if (obj.settings.encoder && obj.settings.encoder.value) {
                  const serverEncoder = obj.settings.encoder.value;
                  if (currentEncoderMode !== serverEncoder) {
                      console.log('Syncing encoder mode from server:', serverEncoder, '(was:', currentEncoderMode + ')');
                      currentEncoderMode = serverEncoder;
                      setStringParam('encoder', currentEncoderMode);
                  }
              }
          ...
          try {
            ...
            console.log('[WS] Received cursor message:', cursorData);
            if (window.webrtcInput && typeof window.webrtcInput.updateServerCursor === 'function') {
                console.log('[WS] Calling updateServerCursor with data:', cursorData);
                window.webrtcInput.updateServerCursor(cursorData);
            } else {
                console.warn('[WS] webrtcInput not available or updateServerCursor not a function');
            }
          } catch (e) {
...
function runPreflightChecks() {
    ...
    if (!window.isSecureContext) {
        console.warn("Non-secure context detected. Continuing without HTTPS enforcement.");
        if (statusDisplayElement) {
            statusDisplayElement.textContent = 'Warning: Insecure HTTP session. Some browsers may limit WebCodecs.';
            statusDisplayElement.classList.remove('hidden');
        }
    }

    if (typeof window.VideoDecoder === 'undefined') {
        console.warn("Browser does not support VideoDecoder. Continuing in JPEG-only mode.");
        if (statusDisplayElement) {
            statusDisplayElement.textContent = 'Warning: VideoDecoder not available. H.264 disabled; JPEG-only mode.';
            statusDisplayElement.classList.remove('hidden');
        }
    }
