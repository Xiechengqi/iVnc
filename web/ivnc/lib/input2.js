...
export class Input {
    constructor(element, send, isSharedMode = false, playerIndex = 0,  useCssScaling = false, initialSlot = null) {
        ...
        this._imeModeActive = false;
        this.keyboardInputAssist = document.getElementById('keyboard-input-assist');

        // Hidden textarea to capture IME composition events.
        // Browsers only fire composition events on editable elements.
        this._imeTextarea = document.createElement('textarea');
        this._imeTextarea.style.cssText = 'position:fixed;left:-100px;top:-100px;width:1px;height:16px;color:transparent;background:transparent;caret-color:transparent;outline:none;border:0;padding:0;margin:0;pointer-events:none;z-index:99999;font-size:16px;line-height:16px;overflow:hidden;resize:none;';
        this._imeTextarea.setAttribute('autocomplete', 'off');
        this._imeTextarea.setAttribute('autocorrect', 'off');
        this._imeTextarea.setAttribute('autocapitalize', 'off');
        this._imeTextarea.setAttribute('spellcheck', 'false');
        document.body.appendChild(this._imeTextarea);
    ...
    toggleImeMode() {
        this._imeModeActive = !this._imeModeActive;
        if (this._imeModeActive) {
            if (this._imeTextarea) this._imeTextarea.focus({ preventScroll: true });
        } else {
            if (this._imeTextarea) {
                this._imeTextarea.blur();
                this._imeTextarea.value = '';
            }
        }
        return this._imeModeActive;
    }
    ...
    _getCursorTarget() {
        return this.cursorElement || this.element;
    }

    _setCursor(value, important = true) {
        const target = this._getCursorTarget();
        if (important) {
    ...
    _updateBrowserCursor() {
        ...
        this._setCursor(`url("${cursorDataUrl}") ${this._rawHotspotX} ${this._rawHotspotY}, default`);
    }
    ...
    async updateServerCursor(cursorData) {
        if (cursorData.override) {
            ...
            this._cursorBase64Data = null;
            this.cursorDiv.style.display = 'none';
            this._useDefaultCursor = cursorData.override !== 'none';
            this._setCursor(cursorData.override);
            return;
        }

        if (parseInt(cursorData.handle, 10) === 0) {
            console.log('[Input] Cursor handle=0, using default cursor');
            this._cursorImageBitmap = null;
            this._cursorBase64Data = null;
            this.cursorDiv.style.display = 'none';
        ...
        if (!cursorData.curdata || this._trackpadMode) {
            console.log('[Input] Hiding cursor - no curdata or trackpad mode');
            this._cursorImageBitmap = null;
            this._cursorBase64Data = null;
            this.cursorDiv.style.display = 'none';
            if (this._useDefaultCursor) {
    ...
    _handleKeyDown(event) {
        ...
        // Let browser dev tools, fullscreen, and refresh shortcuts pass through
        var _isMac = browser.isMac();
        if (
            event.code === 'F12' ||
            event.code === 'F11' ||
            event.code === 'F5' ||
            (event.code === 'KeyR' && ((_isMac && event.metaKey) || (!_isMac && event.ctrlKey))) ||
            (event.code === 'KeyI' && ((_isMac && event.metaKey && event.altKey) || (!_isMac && event.ctrlKey && event.shiftKey))) ||
            (event.code === 'KeyJ' && ((_isMac && event.metaKey && event.altKey) || (!_isMac && event.ctrlKey && event.shiftKey))) ||
            (event.code === 'KeyC' && event.shiftKey && ((_isMac && event.metaKey) || (!_isMac && event.ctrlKey))) ||
            (event.code === 'KeyF' && _isMac && event.metaKey && (event.ctrlKey || event.shiftKey))
        ) {
            return;
        }
        ...
        // Only treat keyCode 229 as IME interception when IME mode is active.
        // Without this guard, system IMEs (e.g. Chinese/Japanese) report 229
        // for ALL keystrokes on contentEditable elements, breaking input.
        if (event.keyCode === 229 && this._imeModeActive) {
            _stopEvent(event);
            return;
        }
        ...
        if (!this._isSynth) {
            for (const code in this._keyDownList) {
                ...
                if ((code === 'ControlLeft' || code === 'ControlRight') && !event.ctrlKey) {
                    // Don't release virtual Ctrl if it came from Cmd remap and Cmd is still held
                    if (this._macCmdSwapped && event.metaKey) continue;
        ...
        // Ctrl+Shift+Space toggles IME mode
        if (event.code === 'Space' && event.ctrlKey && event.shiftKey) {
            const active = this.toggleImeMode();
            if (this.onimetoggle) this.onimetoggle(active);
            _stopEvent(event);
            return;
        }
        if (event.code === 'KeyF' && event.ctrlKey && event.shiftKey && !(_isMac && event.metaKey)) {
        ...
        if (browser.isMac() && code !== "MetaLeft" && code !== "MetaRight" &&
            ...
            } else if (this._macCmdSwapped) {
                // Cmd still held from previous remap (e.g. Cmd+A then Cmd+X).
                // Virtual Ctrl is already down, just ensure it stays pressed.
                if (!this._keyDownList["ControlLeft"]) {
                    this._sendKeyEvent(KeyTable.XK_Control_L, "ControlLeft", true);
                }
            }
        }
        ...
        if ((browser.isMac() || browser.isIOS()) && (code === 'CapsLock')) {
            // macOS uses CapsLock for IME switching (pinyin/english).
            // getModifierState('CapsLock') returns true in pinyin mode,
            // which would incorrectly enable CapsLock on the remote.
            // Suppress CapsLock entirely — IME text goes via composition events.
            _stopEvent(event);
            return;
        }
        ...
        // Clipboard shortcuts (Ctrl+C/V/X or Cmd+C/V/X)
        const isClipboardKey = (code === 'KeyC' || code === 'KeyV' || code === 'KeyX') &&
            (event.ctrlKey || event.metaKey) && !event.altKey && !event.shiftKey;
        if (isClipboardKey) {
            if (code === 'KeyV') {
                var now = Date.now();
                if (!this._lastPasteKeyTime || (now - this._lastPasteKeyTime >= 1000)) {
                    this._lastPasteKeyTime = now;
                    this._sendKeyEvent(keysym, code, true);
                    this._sendKeyEvent(keysym, code, false); // Immediate keyup to prevent remote key repeat
                    if (window.__ivncClipboard) window.__ivncClipboard.enablePaste();
                    // Don't _stopEvent — let browser trigger paste
                } else {
                    _stopEvent(event);
                }
            } else {
    ...
    _compositionUpdate(event) {
        ...
        // Store preedit text but don't send keysyms during composition
    ...
    _compositionEnd(event) {
        ...
        this.compositionString = "";
        if (text) {
            // Send committed text directly via text input channel
            this.send("t," + text);
        }
        // Clear textarea content but keep focus for next IME input
        if (this._imeTextarea) this._imeTextarea.value = '';
    }
    ...
    _mouseButtonMovement(event) {
        ...
        // Update IME textarea position on click.
        if (this._imeTextarea && (event.type === 'mousedown' || event.type === 'pointerdown')) {
            let imeX = event.clientX;
            let imeY = event.clientY;
            if (document.pointerLockElement && this._latestMouseX != null) {
                imeX = this._latestMouseX;
                imeY = this._latestMouseY;
            }
            if (imeX > 0 || imeY > 0) {
                this._imeTextarea.style.left = (imeX - 4) + 'px';
                this._imeTextarea.style.top = (imeY + 4) + 'px';
            }
            // If IME mode is active, keep textarea focused after click
            if (this._imeModeActive) {
                setTimeout(() => {
                    if (this._imeTextarea) this._imeTextarea.focus({ preventScroll: true });
                }, 0);
        ...
        if (this.inputAttached && !this.use_browser_cursors && !this._useDefaultCursor) {
            this.cursorDiv.style.display = this._cursorImageBitmap ? 'block' : 'none';
            this._setCursor('none');
        }
    ...
    setTrackpadMode(enabled) {
        ...
        if (this._trackpadMode || this.use_browser_cursors) {
            if (!this._useDefaultCursor) {
                this._setCursor('none');
                this._setCursor('default', false);
            } else {
                this._setCursor('default');
            }
        } else if (!this._useDefaultCursor) {
            this._setCursor('none');
            this.cursorDiv.style.display = 'none';
        } else {
            this._setCursor('default');
    ...
    _handleTouchEvent(event) {
        ...
        if (type === 'touchstart') {
            if (!this.use_browser_cursors && !this._useDefaultCursor) {
                this.cursorDiv.style.display = this._cursorImageBitmap ? 'block' : 'none';
            }
    ...
    _triggerMouseWheel(direction, magnitude) {
        ...
        const dy = direction === 'up' ? -steps : steps;
        this.send(`w,0,${dy}`);
    }

    _triggerHorizontalMouseWheel(direction, magnitude) {
        const steps = Math.max(1, Math.round(magnitude));
        const dx = direction === 'left' ? -steps : steps;
        this.send(`w,${dx},0`);
    }
    ...
    _windowMath() {
        ...
        // object-fit: fill — video stretches to fill the entire element, no offsets
        const mouseMultiX = frameW / windowW;
        const mouseMultiY = frameH / windowH;
        this.m = {
            mouseMultiX, mouseMultiY, mouseOffsetX: 0, mouseOffsetY: 0,
    ...
    attach_context() {
        ...
        const compositionTarget = this._imeTextarea;
        ...
        // Clear textarea after composition ends to prevent character buildup
        this.listeners_context.push(addListener(compositionTarget, 'input', function(e) {
            if (!this.isComposing) {
                compositionTarget.value = '';
            }
        }, this));
        if (browser.isLinux()) {
