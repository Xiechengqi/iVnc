...
import { Input } from "./input2.js?v=16";
...
export class WebRTCDemo {
	...
	_ontrack(event) {
		...
		if (event.track.kind === "video") {
			...
			this._setDebug("Audio track received, attaching to audio element");
			if (!this._audioElement) {
				this._audioElement = document.createElement('audio');
				this._audioElement.autoplay = true;
				// Start muted to comply with autoplay policy; unmute after user gesture
	...
	/**
	 * Unmutes audio playback. Must be called after a user gesture (click/keypress)
	...
	unmuteAudio() {
		if (this._audioElement) {
			...
			this._audioElement.play().catch((e) => {
				this._setDebug("Audio play failed: " + e.message);
			});
		}
	}
	...
	_onPeerdDataChannel(event) {
		...
		// If the channel is already open, fire the open callback immediately.
