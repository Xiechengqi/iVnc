...
use super::GstError;
use crate::config::{HardwareEncoder, VideoCodec};
...
const VAAPI_ENCODERS: &[EncoderInfo] = &[
    EncoderInfo {
        name: "vaapih264enc",
        encoder_type: HardwareEncoder::Vaapi,
        codec: VideoCodec::H264,
        priority: 90,
    },
    EncoderInfo {
...
const NVENC_ENCODERS: &[EncoderInfo] = &[
    EncoderInfo {
        name: "nvh264enc",
        encoder_type: HardwareEncoder::Nvenc,
        codec: VideoCodec::H264,
        priority: 95,
    },
    EncoderInfo {
...
const QSV_ENCODERS: &[EncoderInfo] = &[
    EncoderInfo {
        name: "qsvh264enc",
        encoder_type: HardwareEncoder::Qsv,
        codec: VideoCodec::H264,
        priority: 92,
    },
    EncoderInfo {
...
impl EncoderSelection {
    ...
    pub fn select(codec: VideoCodec, hw_pref: HardwareEncoder) -> Self {
        ...
        if hw_pref != HardwareEncoder::Auto {
            for encoder in &available {
                if encoder.encoder_type == hw_pref {
                    info!(
                        "Selected requested encoder: {} ({:?})",
                        encoder.name, hw_pref
                    );
                    return Self {
                        info: encoder.clone(),
                    };
            ...
            warn!(
                "Requested encoder type {:?} not available, using best alternative",
                hw_pref
            );
        }
        ...
        info!(
            "Selected encoder: {} (type: {:?}, priority: {})",
            best.name, best.encoder_type, best.priority
        );
    ...
    pub fn create_encoder(
    ...
    ) -> Result<(gst::Element, String), GstError> {
        let encoder = match self.info.name {
            ...
            // VP9 software
            "vp9enc" => gst::ElementFactory::make("vp9enc")
                .name("encoder")
                .property("target-bitrate", bitrate_kbps * 1000)
                .property("keyframe-max-dist", keyframe_interval as i32)
            ...
            // AV1 software
            "av1enc" | "rav1enc" => gst::ElementFactory::make(self.info.name)
                .name("encoder")
            ...
            // VA-API VP8/VP9
            "vaapivp8enc" | "vaapivp9enc" => gst::ElementFactory::make(self.info.name)
                .name("encoder")
                .property("bitrate", bitrate_kbps)
                .property("keyframe-period", keyframe_interval)
                .build(),
            ...
            // VA-API AV1
            "vaapiav1enc" => gst::ElementFactory::make("vaapiav1enc")
                .name("encoder")
                .property("bitrate", bitrate_kbps)
                .build(),
            ...
            // NVIDIA V4L2 H.264
            "nvv4l2h264enc" => gst::ElementFactory::make("nvv4l2h264enc")
                .name("encoder")
            ...
            // Intel QSV H.264
            "qsvh264enc" => gst::ElementFactory::make("qsvh264enc")
                .name("encoder")
                .property("bitrate", bitrate_kbps)
                .property("gop-size", keyframe_interval)
            ...
            // Intel QSV VP9/AV1
            "qsvvp9enc" | "qsvav1enc" => gst::ElementFactory::make(self.info.name)
                .name("encoder")
                .property("bitrate", bitrate_kbps)
                .property("gop-size", keyframe_interval)
                .build(),
            ...
            _ => {
                return Err(GstError::EncoderNotFound(format!(
                    "Unknown encoder: {}",
                    self.info.name
                )));
            }
        ...
        let encoder = encoder.map_err(|e| {
            GstError::EncoderNotFound(format!(
